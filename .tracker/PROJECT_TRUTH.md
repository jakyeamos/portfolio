...
- Historic Shot Clip still has 39 reference-only shot entries without verified clip sources; the modal now avoids selecting those while each zone has at least one provider-backed clip
...
- **Shot embed coverage:** `pnpm shot-embeds` — PASS on 2026-06-23; all court zones have at least one syntactically valid provider-backed clip, with 6 YouTube clips and 39 reference-only shot entries still awaiting verified source data
