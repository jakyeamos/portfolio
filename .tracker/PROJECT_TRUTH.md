...
The portfolio now includes `/blog` as a first-class writing surface for concepts that need more room than project cards. Blog posts are repo-backed Markdown files under `src/content/blog/*.md`, with the initial TMCP concept note migrated out of hardcoded TypeScript content. The owner-facing writer is available only during local development or an explicit private `VITE_ENABLE_BLOG_WRITER=true` build; it emits canonical Markdown plus a BIP handoff payload with portfolio and FRMWRK repo targets so BIP can own draft hosting, review, and cross-repo pushes without exposing public unauthenticated publishing.
